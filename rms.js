...
function CreateStimulusContext(_id, stimulus, stimulus_vertical_flip, frameWidth, frameHeight, stimulus_width,
    ...
    start_compute, fixationWidth, second_stimulus, if_second_stimulus) {
    ...
    let second_stimulus_location;
    if (stimulus_side === 0) {
        stimulus_location = (3 * frameWidth / 4) - (frameHeight / 2);
        second_stimulus_location = (frameWidth / 4) - (frameHeight / 2);
    } else if (stimulus_side === 1) {
        stimulus_location = (frameWidth / 4) - (frameHeight / 2);
        second_stimulus_location = (3 * frameWidth / 4) - (frameHeight / 2);
    } else if (stimulus_side === 2) {
        stimulus_location = 0;
        second_stimulus_location = (frameHeight / 2) + (fixationWidth / 2) + 5;
    } else if (stimulus_side === 3) {
        stimulus_location = (frameHeight / 2) + (fixationWidth / 2) + 5;
        second_stimulus_location = 0;
    }
    ...
    if (stimulus_side === 0) {
        ...
        second_gap = Math.max((frameWidth / 2) - (fixationWidth / 2) - stimulus_width, 0);
    } else if (stimulus_side === 2) {
        gap = Math.max((frameHeight / 2) - (fixationWidth / 2) - stimulus_height, 0);
    } else if (stimulus_side === 3) {
        second_gap = Math.max((frameHeight / 2) - (fixationWidth / 2) - stimulus_height, 0);
    }

    if (if_second_stimulus) {
        let img2 = new Image();
        img2.id = 'stimulusImg2';
        img2.src = second_stimulus;
        img2.onload = function() {
            if (stimulus_side > 1) {
                stimulus_context.drawImage(img2, 0, second_stimulus_location + second_gap,
                    stimulus_width, stimulus_height);
            } else {
                stimulus_context.drawImage(img2, second_stimulus_location + second_gap, 0,
                    stimulus_width, stimulus_height);
            }
        }
    }
